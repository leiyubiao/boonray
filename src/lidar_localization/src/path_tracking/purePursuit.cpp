...
void purePursuit::PID_forSpeed(const float expected_speed, const float actual_speed) // reserved;
{
    ...
    m_error_sum += m_error;
    if (m_error < -1.50f) // 5.4 km/h
    {
        m_cmd.speed = 0.0f;
        m_cmd.brake = m_pid.Kp_brake*m_error +  m_pid.Ki_v*m_error_sum + m_pid.Kd_v*(m_error-m_error_pre);


    }else{
        m_cmd.brake = 0.0f;
        m_cmd.speed = actual_speed + m_pid.Kp_v * m_error + m_pid.Ki_v*m_error_sum + m_pid.Kd_v*(m_error-m_error_pre);
    }

    m_cmd.speed = min(30.0f,max(0.0f,m_cmd.speed));
...
purePursuit::ctrlCommand purePursuit::VehicleControl(const Position &vehiclePos, const int aeb,  const float expected_speed,  const float actual_speed)
	{
        ...
        expected_v = 2.0f;
		if (m_nearestIndex > 20 && m_nearestIndex<40) expected_v = 4.0f;
        if (m_nearestIndex > 40 && m_nearestIndex<m_upperIndex) expected_v = 2.3f;
