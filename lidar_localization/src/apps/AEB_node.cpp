...
int main(int argc, char *argv[])
{
    ...
    //禁区划分
    float carFrontNear = 2.0;
    float carFrontFar = 5.0;
    float carLeft = 0.6;
    float carRight = -0.6;
    ...
    std::shared_ptr<AEB_operator> AEB_operation_ptr = std::make_shared<AEB_operator>(carFrontNear, carFrontFar, carLeft, carRight);
    ...
    while (ros::ok())
    {
        ...
        while (grid_map_data_buff.size() > 0)            //把cloud里面的所有数据都处理干净
        {
            ...
            float delta=8;//度
            float Wheelbase=1.5;
            //AEB_operation_ptr->LidarAEB_AckermanModel(grid_map_data, hasObstacle, speed, delta, Wheelbase);
            AEB_operation_ptr->LidarAEB(grid_map_data, hasObstacle);
